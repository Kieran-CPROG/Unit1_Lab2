...
def select(rats, pups):
  ...
  if rats[0][-1] < rats[1][-1]:
    ...
    smallest = rats[1][-1]
  return newColony, largest, smallest
...
def draw(means, largest, gen, go, finish):
  ...
  print()
  print()
def write_file(large, small, means):
  with open('largeNums.txt', 'w') as file1:
    for x in large:
      file1.write("%s\n" % x)
  with open('smallNums.txt', 'w') as file1:
    for x in small:
      file1.write("%s\n" % x)
  with open('meansNums.txt', 'w') as file1:
    for x in means:
      file1.write("%s\n" % x)
...
def main():
  ...
  while count <= GENERATION_LIMIT and Flag == True:
    ...
    ratColony, large, small = select(ratColony, child)
  ...
  write_file(largest, smallest, averages)
